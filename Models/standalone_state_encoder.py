# Standalone Pokemon State Encoder
# Extracted from neural_mcts.py to eliminate legacy dependencies
...
class PokemonStateEncoder:
    ...
    def encode_battle_state(self, battle: Any) -> torch.Tensor:
        ...
        try:
            ...
            # Feature dimensions (Phase 4 Complete + Type Effectiveness Fix)  
            team_features = 6 * 25  # 6 Pokemon * 25 features each
            field_features = 183    # Field conditions
            type_effectiveness = 18 * 18  # Type effectiveness matrix (324 features)
            game_features = 23      # Turn, timer, etc.
            
            total_features = team_features * 2 + field_features + type_effectiveness + game_features  # = 830
            ...
            # CRITICAL FIX: Encode type effectiveness matrix (18x18 = 324 features)
            offset = self._encode_type_effectiveness_matrix(state_vector, offset)
    ...
    def _encode_type_effectiveness_matrix(self, state_vector: torch.Tensor, offset: int) -> int:
        """Encode complete 18x18 type effectiveness matrix into state vector"""
        try:
            # Import type effectiveness system
            import sys
            from pathlib import Path
            sys.path.append(str(Path(__file__).parent.parent))
            from utils.type_effectiveness import _TYPES, _matrix
            
            # Get the complete type effectiveness matrix
            type_matrix = _matrix()
            
            # Types in order: Normal, Fire, Water, Electric, Grass, Ice, Fighting, Poison, Ground, Flying, Psychic, Bug, Rock, Ghost, Dragon, Dark, Steel, Fairy
            types_order = _TYPES  # ["Normal","Fire","Water","Electric","Grass","Ice","Fighting","Poison","Ground","Flying","Psychic","Bug","Rock","Ghost","Dragon","Dark","Steel","Fairy"]
            
            # Encode 18x18 matrix = 324 features
            for attacking_type in types_order:
                for defending_type in types_order:
                    effectiveness = type_matrix.get(attacking_type, {}).get(defending_type, 1.0)
                    
                    # Normalize effectiveness values: 0.0 = 0x, 0.5 = 0.5x, 1.0 = 1x, 2.0 = 2x
                    # Map to 0-1 range: 0x->0, 0.5x->0.25, 1x->0.5, 2x->1.0
                    normalized_effectiveness = min(effectiveness / 2.0, 1.0)
                    
                    state_vector[offset] = normalized_effectiveness
                    offset += 1
            
            return offset
            
        except Exception as e:
            logging.debug(f"[Neural Encoder] Type effectiveness encoding error: {e}")
            # Fill with neutral values if encoding fails
            for i in range(324):  # 18x18 = 324
                state_vector[offset + i] = 0.5  # Neutral effectiveness
            return offset + 324
